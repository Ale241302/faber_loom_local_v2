...
status: DONE — Ejecutado por Alejandro, servidor operativo
stamp: DONE v1.7 — 2026-04-05
tipo: Lote ruteado (ref → PLB_ORCHESTRATOR §E)
sprint: 22
priority: P0 (camino crítico → bloquea S23, S24)
depends_on: LOTE_SM_SPRINT21 (DONE — ejecutado AG-02, producción 2026-04-01),
...
changelog:
  ...
  - v1.7 (2026-04-01): Alineación con RESUMEN_SPRINT21 real y resumen_artefactos_expedientes (S20B). Cambios: (1) S22-10 corregido — NotificationTemplate no existe, alertas van como EventLog con event_type='margin.alert' + action_source='system_margin_alert'. (2) Estado post-S21 actualizado con implementación real (last_seen_at DateTimeField, no event_id; migraciones 0020-0022; modelo users_mwtuser). (3) action_source contract extendido: +activate_pricelist, +system_recalculate_assignments, +system_margin_alert. (4) S22-06 y S22-09 EventLog con event_type y action_source explícitos. (5) C7 documenta custom_artifact_policy de S20B como contexto sin impacto directo.
...
**Estado post-Sprint 21 (DONE — 2026-04-01):**
- EventLog extendido: +user FK, +proforma FK, +action_source (CharField 32), +previous_status, +new_status (todos nullable)
- UserNotificationState con `last_seen_at` (DateTimeField, NO event_id — cambio vs spec original)
- Activity feed: GET /activity-feed/ (paginado 25, max 100), POST /activity-feed/mark-seen/ (avanza last_seen_at), GET /activity-feed/count/ (slice [:100])
- get_visible_events(user) centralizado en activity_permissions.py
- EventLogFeedSerializer: user_display con fallback full_name→role→'Usuario'→'Sistema'. Nunca payload ni email.
- Role-based sidebar activada: CEO=todo, AGENT_*=Dashboard+Expedientes+Products, CLIENT_*=Portal
- 24 tests en test_activity_feed.py
- action_source values en uso: C1..C22 (dispatcher), create_proforma, reassign_line, change_mode, patch_{status}, system_credit_clock
- Migraciones: 0020+0021+0022 aplicadas (con resolución manual de inconsistencias — ver RESUMEN_SPRINT21)
- Modelo usuario real: users_mwtuser (settings.AUTH_USER_MODEL)
...
### C6. Extensión del contrato action_source (S21)

S21 cerró el contrato de `action_source` con valores: C1..C22, create_proforma, reassign_line, change_mode, patch_{status}, system_credit_clock. S22 extiende el contrato con 3 valores nuevos:

| action_source | Origen | user |
|---------------|--------|------|
| `activate_pricelist` | S22-06 activate_pricelist() | request.user |
| `system_recalculate_assignments` | S22-09 Celery task | None |
| `system_margin_alert` | S22-10 alerta margen | None |

Estos valores se agregan a `CEO_ONLY_EVENT_TYPES` si corresponde: `margin.alert` NO se agrega (el CEO debe verlo). `pricelist.activated` y `assignments.recalculated` NO se agregan (son operacionales, visibles para CEO y AGENT_*).

### C7. custom_artifact_policy (S20B — solo contexto)

S20B implementó `custom_artifact_policy` que permite al admin override la policy de artefactos por expediente. S22 no toca artefactos ni la policy — solo pricing. Pero si en futuro el pricing engine necesita interactuar con gates de artefactos (ej: bloquear avance por MOQ), la integración es vía C5 gate (ya documentado en S22-07), no vía custom_artifact_policy.
...
- **Qué hacer:**
  ...
  4. EventLog: registrar activación y cada extinción con `event_type='pricelist.activated'`/`'pricelist.extinct'`, `action_source='activate_pricelist'`, `user=request.user`. Payload: `{ version_activated, versions_extinct: [...], versions_kept: [...], reason }`
- **Criterio de done:**
...
- **Qué hacer:**
  ...
  5. EventLog al completar: `event_type='assignments.recalculated'`, `action_source='system_recalculate_assignments'`, `user=None` (Celery task, patrón S21 `system_credit_clock`). Payload: cuántos CPAs recalculados, cuántos cambiaron de precio
- **Criterio de done:**
...
- **Qué hacer:**
  ...
  2. Si `margin < Brand.min_margin_alert_pct` → crear **EventLog** con `event_type='margin.alert'`, `action_source='system_margin_alert'`, `user=None`. S21 implementó observabilidad vía EventLog + activity feed (NO hay NotificationTemplate — se eliminó esa referencia errónea).
  3. Solo si Brand.min_margin_alert_pct no es NULL (alerta activa)
  4. Payload del EventLog: `{ sku, client, cached_price, estimated_cost, margin_pct, threshold_pct }`
  5. La alerta aparece automáticamente en el activity feed del CEO (S21 — `get_visible_events` la incluye porque CEO ve todo)
- **Criterio de done:**
  - [ ] EventLog con event_type='margin.alert' se crea cuando margen < threshold
  - [ ] No se genera si threshold es NULL
  - [ ] Payload tiene toda la info para que el CEO decida
  - [ ] Alerta visible en activity feed del CEO
