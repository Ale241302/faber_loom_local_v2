# AUDIT_FABERLOOM_A1_SPEC_CANON_v1 — SPEC Canonical Inventory — FaberLoom v1 Beta Blueprint
id: AUDIT_FABERLOOM_A1_SPEC_CANON_v1
version: 1.0
status: DRAFT
visibility: [INTERNAL]
...
aplica_a: [FaberLoom]
relacionado: SPEC_FABERLOOM_ARCHITECTURE_v1_BLUEPRINT.md · AUDIT_FABERLOOM_A4_ARCH_PRINCIPLES_CANON_v1.md · AUDIT_FABERLOOM_A5_KNOWLEDGE_FLOW_CANON_v1.md
...
**[NOT IN SPEC]** — the word "skill" never appears. Only adjacents: `overlay_policy` table (§2 row 16: "policies scope-override") and cross-reference in §16 line 956 — "4 scopes + 3 roles + TTL 90d learned overlays + leakage CI gate" (references `SPEC_USER_ADMIN_KNOWLEDGE_FLOW.md`). No writer/approver/mutability/TTL per layer given. Human-gate phrase for skills **[NOT IN SPEC]**; closest phrase used for workflow gating is **"draft-first absoluto"** (§7.1).
...
**Promotion / supersede:** `memory_chunk.promoted`, `memory_chunk.revoked`, `memory_chunk.superseded` exist as audit verbs (§12); `supersedes_chunk_id` implements lineage; UPDATE limited to status `active → superseded|revoked`. **Cross-scope promotion mechanics [NOT IN SPEC]** in this blueprint (lives in `SPEC_USER_ADMIN_KNOWLEDGE_FLOW.md`).
...
2. Skill 3-layer (base/manual/learned) model → expected in `SPEC_USER_ADMIN_KNOWLEDGE_FLOW.md`
