...
id: ENT_PLAT_DESIGN_TOKENS
status: APROBADO — Sprint 3
visibility: [INTERNAL]
domain: Plataforma (IDX_PLATAFORMA)
version: 1.1
refs: ENT_PROD_GOL.E, ENT_MARCA_IDENTIDAD, ENT_COMP_CONTENT_RULES.C, POL_ANTI_CONFUSION, rw_sticker_v9.html, PLB_COPY, SCH_PROFORMA_MWT
aplica_a: [SHARED]
...
| ENT_COMP_CONTENT_RULES.C | WCAG contraste verificado en todas las combinaciones |
...
## J — Token Set ART-02 (Proforma MWT)

Las proformas usan un `:root` compacto que mapea 1:1 a los tokens del sistema. Este es el mapping canónico:

| Variable ART-02 | Token sistema | Hex | Uso en proforma |
|-----------------|---------------|-----|-----------------|
| --navy | --brand-primary | #013A57 | Topbar, h2, totales, pills qty, arb-tot bg |
| --navy-h | --interactive-hover | #0a4d6e | Hover navy |
| --mint | --brand-accent | #75CBB3 | No usado directamente (heredado de sistema) |
| --ice | --brand-ice | #A8D8EA | arb-tot label text |
| --ice-s | — | #EDF6FB | Derivado ice 10% |
| --bg | --bg | #F8FAFB | Body background |
| --srf | --surface | #FFFFFF | Cards, secciones, pills |
| --raised | --surface-raised | #F1F5F9 | thead, trow, arb-card, notes-card, sr border |
| --brd | --border | #E2E8F0 | Card borders, pill borders, sect borders |
| --brd2 | --border-strong | #CBD5E1 | Button outline border |
| --t1 | --text-primary | #0F172A | Body text, strong en notes |
| --t2 | --text-secondary | #475569 | Labels (.k), meta, descripciones |
| --t3 | --text-tertiary | #94A3B8 | thead th, pill labels (.pl), arb-card product |
| --ok | --success | #0E8A6D | Revenue, delta positivo, comisión bruta |
| --ok-bg | --success-bg | #F0FAF6 | No usado en proforma |
| --warn | --warning | #B45309 | Badge BORRADOR, cadena determinista PENDIENTE |
| --warn-bg | --warning-bg | #FFF7ED | Badge BORRADOR fondo |
| --crit | --critical | #DC2626 | Badge CEO-ONLY, botón descartar |
| --crit-bg | --critical-bg | #FEF2F2 | Badge CEO-ONLY fondo |
| --info | --info | #0369A1 | Card headers vista Marluvas (cliente, condiciones) |
| --purple | — | #7C3AED | Comisión %, promo tags (específico ART-02) |
| --purple-bg | — | #F5F3FF | Fila promo, badge promo |

**Nota:** --purple y --purple-bg son específicos de ART-02 (representan comisión/promo). No están en el token set general del sistema. Si otros artefactos necesitan un color de comisión, debería promoverse a token de sistema.

### Tipografía en ART-02

| Elemento | Font | Size | Weight |
|----------|------|------|--------|
| Topbar h1 | Plus Jakarta Sans | 13px | 700 |
| Head h2 | Plus Jakarta Sans | 18px | 800 |
| Head .meta | Plus Jakarta Sans | 12px | 400 |
| Card-h h3 | Plus Jakarta Sans | 12px | 700 uppercase |
| Sect-h h3 | Plus Jakarta Sans | 12px | 700 uppercase |
| .sr .k (label) | Plus Jakarta Sans | 12px | 400 |
| .sr .v (valor) | Plus Jakarta Sans | 12px | 700 tabular-nums |
| .sr.big .v | Plus Jakarta Sans | 18px | 700 |
| Código (.m) | JetBrains Mono | 11px | 400 |
| Pill .s (talla) | Plus Jakarta Sans | 9px | 600 |
| Pill .q (qty) | JetBrains Mono | 13px | 700 |
| .pl (pill label) | Plus Jakarta Sans | 10px | 700 uppercase |
| thead th | Plus Jakarta Sans | 9.5px | 700 uppercase |
| tbody td | Plus Jakarta Sans | 12px | 400 |
| .arb-tot .tv | Plus Jakarta Sans | 20px | 800 |
| .notes-card | Plus Jakarta Sans | 11px | 400 |
...
Estado: VIGENTE
Aprobador final: CEO
Origen: sticker v9 + ENT_PROD_GOL.E + ENT_MARCA_IDENTIDAD + POL_ANTI_CONFUSION + ENT_COMP_CONTENT_RULES.C

Changelog:
- v1.0 (2026-02-27): Creación inicial. 9 secciones A-I.
- v1.1 (2026-03-16): +Sección J (Token Set ART-02). Mapping :root proforma a tokens sistema. Ref: SCH_PROFORMA_MWT.N1.
