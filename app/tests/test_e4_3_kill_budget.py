"""E4-3 kill switch and budget enforcement tests."""
...
def _simple_plan(cost_usd: float = 0.001) -> DispatchPlan:
    return DispatchPlan(
        steps=[
            DispatchStep(
                step_id="s0",
                ...
                chosen={
                    "provider_slug": "fake",
                    "model": "fake-model",
                    "cost_input_1k": cost_usd,
                    "cost_output_1k": cost_usd,
                },
                requires_hitl=False,
            ),
        ],
        est_total_cost_usd=cost_usd,
        planner_cost_usd=0.0,
    )


def test_api_kill_task(client: TestClient) -> None:
    ...
    task = TaskOrchestrator(ctx, conn).run_task_from_plan(
        chat_id=None,
        user_request="kill api test",
        plan=_simple_plan(),
    )
    assert task["status"] == "completed"

    resp = client.post(
        f"/api/workspaces/{ws}/agent-tasks/{task['id']}/kill",
...
def test_budget_overrun_degrades_task(client: TestClient) -> None:
    ws = _setup_catalog(client)
    conn = connect()
    ctx = Context(workspace_id=ws, tenant_id="default", user_id="test", actor_id="test")
    ...
    task = TaskOrchestrator(ctx, conn).run_task_from_plan(
        ...
        plan=_simple_plan(cost_usd=0.001),
    )
