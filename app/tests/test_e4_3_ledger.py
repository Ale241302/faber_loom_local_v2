"""E4-3 ledger/usage task_id linkage tests."""
...
def test_usage_records_linked_to_task(client: TestClient) -> None:
    ...
    task = TaskOrchestrator(ctx, conn).run_task_from_plan(
        chat_id=None,
        user_request="ledger linkage",
        plan=plan,
    )

    rows = conn.execute(
        "SELECT task_id FROM usage_record WHERE workspace_id = ? AND task_id IS NOT NULL",
        (ws,),
    ).fetchall()
    assert any(r["task_id"] == task["id"] for r in rows)
