"""E4-5 — CAPA 1 personal memory must never cross users or workspaces."""
...
def test_proposals_and_blocks_are_isolated_per_user(client: TestClient) -> None:
    ws = _workspace_id(client)
    for _ in range(3):
        chat_id, message_id = _assistant_message_id(client, ws, user="alice")
        client.post(
    ...
    index = client.post(f"/api/workspaces/{ws}/memory/index", headers=_headers("alice")).json()
    proposal_id = index["created"][0]
    client.post(f"/api/workspaces/{ws}/memory/proposals/{proposal_id}/apply", headers=_headers("alice"))

    # Bob cannot see Alice's proposals.
    bobs = client.get(f"/api/workspaces/{ws}/memory/proposals", headers=_headers("bob")).json()
    assert bobs == []

    # Bob cannot recall Alice's blocks at the DB layer.
    with connect() as conn:
        ctx_bob = Context(workspace_id=ws, tenant_id="default", user_id="bob")
        blocks = recall_personal_blocks(ctx_bob, conn)
        assert blocks == []


def test_detector_groups_by_user_not_workspace(client: TestClient) -> None:
    ws = _workspace_id(client)
    # Alice: 3 rejections too_long
    for _ in range(3):
        chat_id, message_id = _assistant_message_id(client, ws, user="alice")
        client.post(
            f"/api/workspaces/{ws}/chats/{chat_id}/messages/{message_id}/feedback",
    ...
    # Bob: 2 rejections too_long (below threshold)
    for _ in range(2):
        chat_id, message_id = _assistant_message_id(client, ws, user="bob")
        client.post(
            f"/api/workspaces/{ws}/chats/{chat_id}/messages/{message_id}/feedback",
            json={"outcome": "rejected", "reason": "too_long"},
    ...
    # Run detector as alice; should only produce a proposal for alice.
    with connect() as conn:
        ctx_alice = Context(workspace_id=ws, tenant_id="default", user_id="alice", actor_id="alice", actor_role_at_decision="owner")
        findings = detect_personal_memory_patterns(ctx_alice, conn)
        user_ids = {f.evidence_json["user_id"] for f in findings}
