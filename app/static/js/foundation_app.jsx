/* ═══════════════════════════════════════════════════════════════
   ...
   Orquesta el gating: bootstrap wizard (M07) → login/2FA (M08) → vistas.
...
function FoundationSection() {
  ...
  const refreshMe = useCallback(async () => {
    if (!F.getSession()) { setMe(null); return; }
    try { setMe(await F.api.get("/auth/me")); }
    catch (e) { setMe(null); }
  }, []);

  useEffect(() => { refreshStatus(); }, [refreshStatus, sessionTick]);
  useEffect(() => { refreshMe(); }, [refreshMe, sessionTick]);
  useEffect(() => {
    const onExpired = () => { setMe(null); setSessionTick((t) => t + 1); };
    window.addEventListener("fnd-session-expired", onExpired);
    return () => window.removeEventListener("fnd-session-expired", onExpired);
  }, []);

  const onAuthed = useCallback(() => { setSessionTick((t) => t + 1); }, []);
  const onLogout = useCallback(async () => {
    try { await F.api.post("/auth/logout"); } catch (e) {}
    F.setSession(null);
    setMe(null);
  ...
  if (!status.bootstrapped) {
    ...
    if (wizard) {
      ...
      return <Wizard api={F.api} status={status} onDone={() => setSessionTick((t) => t + 1)} />;
    }
    return <div style={{ padding: 24, color: "var(--text-muted)" }}>Bootstrap Wizard (M07) no cargado.</div>;
  }

  // 2) Sin sesión → Login / 2FA (M08)
  if (!me) {
    const login = F.views.find((v) => v.id === "m08-login");
    if (login) {
      const Login = login.component;
      return <Login api={F.api} onAuthed={onAuthed} />;
    }
    return <div style={{ padding: 24, color: "var(--text-muted)" }}>Login (M08) no cargado.</div>;
  }
    ...
    <div style={{ display: "grid", gridTemplateColumns: "218px minmax(0, 1fr)", gap: 16, minHeight: 0, height: "100%" }}>
      <div style={{ display: "flex", flexDirection: "column", gap: 4, overflow: "auto", paddingRight: 4 }}>
        <div style={{ padding: "10px 10px 12px" }}>
          <div style={{ fontWeight: 650, fontSize: 13 }}>Foundation Beta</div>
          ...
          <button type="button" onClick={onLogout} style={Object.assign({}, window.Foundation.ui.buttonStyle, { marginTop: 10, width: "100%" })}>Cerrar sesión</button>
        </div>
        {groups.map((g) => {
      ...
      <div style={{ minWidth: 0, overflow: "auto" }}>
        {current
          ? <current.component api={window.Foundation.api} me={me} refreshMe={refreshMe} tenant={status.tenant} />
