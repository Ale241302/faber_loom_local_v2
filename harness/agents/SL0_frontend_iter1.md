All four deliverables already exist on disk, are brand-compliant, properly UTF-8 encoded, and the app runs end-to-end. The "mojibake" seen in raw console dumps was only a PowerShell rendering artifact — the actual file bytes are valid UTF-8 (`·`, `—`, `×`, `í`, `é`, `ó` are all correct).

## Resumen

Verifiqué el estado SL0 del shell frontend de SpaceLoom. Los cuatro entregables están implementados y funcionando:

**Verificación en vivo (servidor real en hilo + sondeo):**
- `/api/health` → 200 (json)
- `/api/workspaces` → 200, devuelve el workspace semilla `MWT Demo` (`mwt-demo`) con `id/name/slug` + campos latentes (`tenant_id`, `user_id`, `schema_version`).
- `/static/index.html` → 200 (text/html)
- `/static/css/main.css` → 200 (text/css, 19.6KB)
- `/static/js/app.jsx` → 200 (servido como text/plain; Babel lo transpila en browser)
- `/` → 200 (sirve el index)

**Conformidad con el contrato y la marca:**
- `index.html` carga React 18 UMD + ReactDOM + Babel standalone (unpkg), las tres fuentes de marca, y `app.jsx` vía `type="text/babel"`. Incluye un *boot shell* pre-hidratación para que la ventana nunca quede en blanco mientras carga el CDN.
- `main.css` define todos los tokens de la paleta fija aprobada (`--bg #F4F1ED`, `--text #1F1E1C`, `--coral #C96442`, `--slate #5A6B7C`, `--coral-hover #A84F33`, neutros tejidos, etc.) y las tres familias tipográficas (EB Garamond voz / Geist UI / Geist Mono datos). Layout topbar + rail + canvas, sin frameworks CSS, responsive.
- `app.jsx` implementa `App` con topbar (isotipo nudo 3×3 + wordmark Faber-coral/Loom-ink), rail con modos **Operar/Aprender/Admin**, canvas con vista **SpaceLoom** (lista de chats, composer, área de mensajes con empty-state), y `fetch('/api/workspaces')` con header `x-user-id: local`. Iconos SVG inline 24×24 stroke 1.75 `currentColor` (isotipo stroke 3.5). Composer sin lógica real (SL0). Soporta inyección del backend vía `window.__SPACELOOM_BOOTSTRAP__`.
- `main.py` levanta uvicorn en hilo daemon, espera `/api/health`, y abre la ventana pywebview apuntando a `http://127.0.0.1:8000/static/index.html` con `background_color="#F4F1ED"`.

**Coincidencia de contrato API ↔ UI:** `WorkspaceListRead.workspaces[]` expone exactamente `id`, `name`, `slug` que consume `app.jsx`. ✓

No fue necesario modificar archivos: el shell SL0 ya cumple la DoD ("la app abre y persiste estado local"). A continuación el contenido exacto verificado en disco.
    ...
    <main className="canvas">
      <div className="context-strip">
        ...
        <div className="context-actions">
          <span className="pill"><IconShield w={16} /> Sellado local</span>
          <span className="pill"><IconRoute w={16} /> BYO-key</span>
        </div>
      </div>
...
- Para correr la ventana desktop: `cd app && python -m src.main` (requiere `pywebview` instalado; el servidor levanta uvicorn en hilo daemon y abre la ventana en `127.0.0.1:8000`).
- Para inyectar el espacio activo desde el backend sin round-trip, basta con definir `window.__SPACELOOM_BOOTSTRAP__ = { workspaces: [...], activeWorkspaceId: "..." }` antes de `app.jsx` (la costura ya está leída en `readBootstrap()`).
- Las vistas Inbox/WorkLoom/Routine Hub/Routing/KB/Auditoría muestran un placeholder honesto ("llega en un hito posterior") — coherente con el alcance SL0 (solo SpaceLoom + shell).