...
from apps.drafts.models import DraftStatus
...
@pytest.mark.django_db
def test_oscillation_counter_fires_after_n_clean_approvals(
    tenant, task, user, captured_events, monkeypatch
):
    limit = 3
...
def test_channel_failure_goes_to_approved_pending_send(tenant, task, user, monkeypatch):
    ...
    try:
        draft = DraftService.generate(task)
        monkeypatch.setattr(
            "apps.drafts.channel_sender.send_mail",
            lambda *a, **k: (_ for _ in ()).throw(Exception("SMTP down")),
        )
        DraftService.approve(draft, user, "operator")
